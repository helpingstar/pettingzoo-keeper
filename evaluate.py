...
class SharedEvaluater:
    def __init__(self, experiment_name: str, dir_name: str) -> None:
        ...
        for agent_type, weights in all_weights.items():
            self.agents[agent_type] = [
                PPOAgent(f"Evaluater_{agent_type}").to(self.device).eval()
        ...
        for agent_type, agents in self.agents.items():
            for i in range(len(agents)):
                utils.load_weights(
                    self.agents[agent_type][i], all_weights[agent_type][i]
                )
        ...
        self.agent_to_idx = {
            agent: i for i, agent in enumerate(config.agent["agent_type"])
        }
    ...
    def fight(
        ...
        winning_count = self.winning_board[self.agent_to_idx[agent1_type]][
            self.agent_to_idx[agent2_type]
        ][agent1_weight_idx][agent2_weight_idx]
        cumulated_score = self.score_board[self.agent_to_idx[agent1_type]][
            self.agent_to_idx[agent2_type]
        ][agent1_weight_idx][agent2_weight_idx]
