...
class SelfPlay:
    def __init__(self, experiment_name: str, train_shared=True, train_independent=True) -> None:
        ...
        if self.train_shared:
            self.shared_main_agents = {
                agent_type: PPOAgent(f"shared {agent_type} main").to(self.device) for agent_type in self.agent_type
            }
            self.shared_saved_agents = PPOAgent(f"shared_None_saved").to(self.device).eval()
        if self.train_independent:
            ...
            self.independent_saved_agents = PPOAgent(f"independent_None_saved").to(self.device).eval()

    def get_env(self):
        self.envs: Dict[str, PikaZooPPOStage] = dict()
        for i, agent_type in enumerate(self.agent_type):
            self.envs[agent_type] = PikaZooPPOStage(agent_type, config.agent["reward"][i])
    ...
    def check_number_of_weights(self, check_shared=True, check_independent=True):
        ...
        first_path = os.path.join(self.experiment_name, config.agent["agent_type"][0], "shared")
    ...
    def init_weights(self):
        ...
        for agent_type in self.agent_type:
            ...
            if self.train_independent:
                ...
                utils.save_weights(network, os.path.join(path_independent, "saved_001.pth"))
    ...
    def load_main_agents(self):
        if self.train_shared:
            shared_main_weights = utils.get_all_agents_weights(self.experiment_name, True, True)
        if self.train_independent:
            independent_main_weights = utils.get_all_agents_weights(self.experiment_name, False, True)

        for agent in self.agent_type:
            if self.train_shared:
                utils.load_weights(self.shared_main_agents[agent], shared_main_weights[agent][0])
            if self.train_independent:
                utils.load_weights(
    ...
    def cycle1(self):
        ...
        shared_saved_weights = utils.get_all_agents_weights(self.experiment_name, True, False)
        # load independent saved
        independent_saved_weights = utils.get_all_agents_weights(self.experiment_name, False, False)
        ...
        for agent_train_idx, agent_train in enumerate(self.agent_type):
            ...
            for div_idx in range(config.selfplay["weight_divison"]):
                for agent_opp_idx, agent_opp in enumerate(self.agent_type):
                    run_name = utils.get_run_name(div_idx, agent_opp, agent_opp_idx, schedule)
                    utils.load_weights(
                        self.shared_saved_agents,
                        shared_saved_weights[agent_opp][schedule[agent_opp_idx][div_idx]],
                    )
            ...
            for div_idx in range(config.selfplay["weight_divison"]):
                for agent_opp_idx, agent_opp in enumerate(self.agent_type):
                    ...
                    utils.load_weights(
                        ...
                        independent_saved_weights[agent_train][schedule[agent_opp_idx][div_idx]],
                    )
    ...
    def cycle2(self, n):
        ...
        # load shared saved(opponent)
        shared_saved_weights = utils.get_all_saved_weights_list(self.experiment_name, True)
        ...
        for agent_train_idx, agent_train in enumerate(self.agent_type):
            ...
            for div_idx in range(config.selfplay["weight_divison"]):
                ...
                _, agent_opp, _, file_name = utils.get_info_by_path(shared_saved_weights[sche_idx])
                print(f"    [main {agent_train} VS {agent_opp}_{file_name} / {sche_idx}]")
                run_name = f"{div_idx:03d}_{agent_opp}_{sche_idx}"
                utils.load_weights(self.shared_saved_agents, shared_saved_weights[sche_idx], 2)
                self.envs[agent_train].train(
                    self.shared_main_agents[agent_train],
                    self.shared_saved_agents,
                    run_name,
    ...
    def cycle3(self, n):
        print(f"[Start cycle {n:03d}]")
        self.load_main_agents()
        schedule = utils.get_schedule2(config.agent["n_agent_type"], self.n_weights)
        print(f"Schedule: {schedule}")
        # load shared saved(opponent)
        shared_saved_weights = utils.get_all_saved_weights_list(self.experiment_name, True)
        print(f"Number of weights: {len(shared_saved_weights)}")
        for agent_train_idx, agent_train in enumerate(self.agent_type):
            ...
            sche_idx = schedule[agent_train_idx]
            _, agent_opp, _, file_name = utils.get_info_by_path(shared_saved_weights[sche_idx])
            print(f"    [main {agent_train} VS {agent_opp}_{file_name} / {sche_idx}]")
            run_name = f"{agent_opp}_{sche_idx}"
            utils.load_weights(self.shared_saved_agents, shared_saved_weights[sche_idx], 2)
            self.envs[agent_train].train(
                self.shared_main_agents[agent_train],
                self.shared_saved_agents,
